...
//  BlueShell
//
//  Created by WnnWoo on 2019/1/23.
//  Copyright © 2019年 WnnWoo. All rights reserved.
//

import Foundation

func testErrorHandle() -> Bool {
    
    //There are several ways to handle errors. One way is to use do-catch. Inside the do block, you mark code that can throw an error by writing try in front of it. Inside the catch block, the error is automatically given the name error unless you give it a different name.
    
    do {
        let printerResponse = try send(job: 1040, toPrinter: "Never")
    ...
    do {
        let printerResponse = try send(job: 1440, toPrinter: "outOfPaper")
        print(printerResponse)
    ...
    let bRet = fridgeContains("banana")
    print(fridgeIsOpen)
    print(bRet)
    // Prints "false"
    return true;
